...
const getAllUserCommunities = async (req, res) => {
	...
	const user = await (req.auth.role === "user" ? User : Doctor).findById(req.auth.id);
	if (!user) {
...
const joinCommunity = async (req, res) => {
	...
	const result = await (req.auth.role === "user" ? User : Doctor).findById(req.auth.id);
...
const createCommunity = async (req, res) => {
	...
	const user = await Doctor.findById(req.auth.id);
	if (!user) {
		return res.status(401).json({ message: "Unauthorized" });
	}
...
const leaveCommunity = async (req, res) => {
	...
	try {
		const Model = req.auth.role === "user" ? User : Doctor;
		const user = await Model.findById(req.auth.id);
		if (!user) {
			return res.status(401).json({ message: "Unauthorized" });
		}
		const { id } = req.body;
		...
		await community.updateOne({
			$pull: { members: req.auth.id },
		});
		await user.updateOne({
			$pull: { communities: id },
		});
		const updatedUser = await Model.findById(req.auth.id);
